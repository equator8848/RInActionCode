...
# 优化一波，给分组列命名然后删除多余的列
# 这里的删除列和分组是独立的，分组还是使用原本的mtcars数据
aggdata2 <- aggregate(mtcars[-c(2, 10)],
  by = list(Cylinders = mtcars$cyl, Gears = mtcars$gear),
  FUN = mean, na.rm = TRUE
)

# 这样会报错：Error in aggregate.data.frame(dataset, by = list(Cylinders = dataset$cyl,  : arguments must have same length
